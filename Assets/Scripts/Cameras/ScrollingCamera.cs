...
public class ScrollingCamera : MonoBehaviour
{
	...
	public static string currentParent = "TitlePath";
	...
	void Update()
	{
		...
		if (currentNode < cameraNodes.Count - 1)
		{
			...
			float coefficient = Mathf.SmoothStep(0, 1, Mathf.SmoothStep(0, 1, elapsedTime / totalTime));
	...
	{
		currentNode = cameraNodes.Count - 1;
		
		if (currentNode > 0)
		{
			transform.position = cameraNodes[currentNode].transform.position;
			transform.rotation = cameraNodes[currentNode].transform.rotation;
			Update();
		}
		else
		{
			Debug.Log("This should never happen (I can't believe I'm writing this)");
		}
	}
}
